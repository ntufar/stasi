...
Version: 0.37 | Date: 2026-05-15 | Author: Nicolai Tufar
...
11. **Arrival alerts (local notifications):** one-shot reminders tied to a **live arrival row** (stop + route + vehicle id from OASA):
    ...
    - **Foreground list:** while Arrivals is visible, the screen **refreshes arrivals every 30 seconds** (forced network each tick) and **on each `RESUMED`** lifecycle (`refreshNow`, forced network), independent of the worker; the list also **recomputes displayed minutes from the snapshot time** between polls (see item 3).
